...
//************************************************ 
{
  ...
  for (int i=0; i<256; i++) {
	double angle = double(i)*(1.0/256.0)*constants::pi;
...
//**********************************************
{
  ...
  const float tmp=(1.0f/constants::pi)/(np.dist2[0]);    // estimate of density
...
//***************************
{
  ...
  int theta = int( acos(dir[2])*(256.0/constants::pi) );
  if (theta>255)
    node->theta = 255;
  else
   node->theta = (unsigned char)theta;

  int phi = int( atan2(dir[1],dir[0])*(256.0/(2.0*constants::pi)) );
  if (phi>255)
    node->phi = 255;
  else if (phi<0)
