...
static glm::vec3 ColorMatchingCurve[81] = {
...
glm::vec3 SpectrumToRGB(float spectralRadiance[WAVELENGTHS], ColorSystem colorSystem)
{
	...
	if (sum > constants::epsilon) color = color / sum;
    ...
    glm::vec3 rgb = glm::vec3((rx * color.x) + (ry * color.y) + (rz * color.z),
...
glm::vec3 GammaCorrect(glm::vec3 rgb, ColorSystem colorSystem)
{
    ...
    else
    {
        ...
        return glm::vec3(powf(rgb.x, power), powf(rgb.y, power), powf(rgb.z, power));
    }
}