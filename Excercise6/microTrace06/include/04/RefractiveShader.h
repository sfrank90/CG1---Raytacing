...
public:
  ...
  double reflection_attenuation;
  double indexOutside;
  double indexInside;

  RefractiveShader(Scene *scene, Vec3f reflection_color = Vec3f(0.5, 0.5, 0.5), // bg
  double reflection_attenuation = 0.4, double indexInside = 1, double indexOutside = 1) // refraction coefficients.
      : Shader(scene), reflection_color(reflection_color), reflection_attenuation(reflection_attenuation), indexOutside(indexOutside), indexInside(indexInside)
  {
  }
  ...
  virtual Vec3f shadeRay(Ray &ray)
  {
    ...
    if (ray.recursionLevel <= MAX_RECURSION_LEVEL)
    {
      ...
      else
      {
        ...
        // Return mirrored reflection_color.
        ret_val = scene->rayTrace(reflect_ray);
        ret_val = componentProduct(ret_val, Vec3f(reflection_attenuation));
        ret_val = componentProduct(ret_val, reflection_color);
      }
