  ...
  virtual void render(const char *fileName, const double task)
  {
    ...
    for (int y = 0; y < camera->resY; y++)
    {
      for (int x = 0; x < camera->resX; x++)
      {
        ...
        //     ray. The ::findIntersection(Ray &ray) method returns true  if the
        //     object is currently closest the camera. In this case you have to
        //     change the pixel color to the object color.
    ...
    printf("\n(MicroTrace): Rendering task %.2f done.\n", task);
